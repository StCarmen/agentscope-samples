    ...
    def _extract_schema_from_original_file(self, file_path: str) -> dict:
        ...
        def extract_schema_from_table(df: pd.DataFrame, df_name: str) -> dict:
            ...
            for col in df.columns:
                ...
                samples = df[col].sample(5).astype(str).tolist()
                col_list.append({
                    "column name": col,
        ...
        def extract_excel(file_source: str, file_name):
            ...
            for sheet_name in excel_file.sheet_names:
                ...
                df = pd.read_excel(file_source, sheet_name=sheet_name, nrows=100).convert_dtypes()
                ...
                if rows:
                    for i, col in enumerate(columns):
                        ...
                        for row in rows:
                            if i < len(row):
                                val = row[i]
                                sample_values.append(str(val) if val is not None else "NULL")
